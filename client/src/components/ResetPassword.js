...
function ResetPassword() {
  ...
  async function handleResetPassword(e) {
    ...
    setLoading(true);
    try {
      const response = await axios.post(URL + "/api/resetpassword/" + resetPasswordToken, password);
    ...
    <div className="w-full max-w-sm p-4 bg-white border border-gray-200 rounded-lg shadow sm:p-6 md:p-8 dark:bg-gray-800 dark:border-gray-700 m-4">
      <form className="space-y-6" onSubmit={handleResetPassword}>
        ...
        <div className="text-sm font-medium text-gray-500 dark:text-gray-300">
          Go to{' '}
          <Link to="/signin" className="text-blue-700 hover:underline dark:text-blue-500">Sign in</Link> page
        </div>
      </form>
    </div>
  );
